...
FILEPATH="./perf/"
CSVPATH="./perf/csv/"
DEFAULT="./perf/csv/default/blackscholes_default1"
PDFPATH="./graphs/perf/"
APPROXIMATION = [
    # "default",
    "fast",
...
def plot_graph(default_results, results, title, approx, threshold=None):
    ...
    plt.title(f'Blackscholes Speedup {title}', fontsize=35)
    ...
    plt.tight_layout()
    if threshold == None:
        plt.savefig(PDFPATH + "blackscholes_" + approx + '.pdf')
    else:
        plt.savefig(PDFPATH + "blackscholes_" + approx + "_" + threshold + '.pdf')
