...
FILEPATH="./perf/"
CSVPATH="./perf/csv/"
DEFAULT="./perf/csv/default/particlefilter_default1"
PDFPATH="./graphs/perf/"
APPROXIMATION = [
    # "default",
    "fast",
...
def plot_graph(default_results, results, title, approx, threshold=None):
    ...
    plt.title(f'Particle Filter Speedup {title}', fontsize=35)
    plt.xlabel('Threads', fontsize=32)
    plt.ylabel('Speedup (ms)', fontsize=32)
    plt.grid(True)
    ...
    plt.tight_layout()
    if threshold == None:
        plt.savefig(PDFPATH + "particlefilter_" + approx + '.pdf')
    else:
        plt.savefig(PDFPATH + "particlefilter_" + approx + "_" + threshold + '.pdf')
