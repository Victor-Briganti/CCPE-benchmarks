...
#pragma omp approx memo shared(xe, ye)
    for (x = 0; x < Nparticles; x++) {
      xe += arrayX[x] * weights[x];
      ye += arrayY[x] * weights[x];
    }

    double xe_value = xe - (int)roundDouble(IszY / 2.0);
    if (xe_value < 0)
      xe_value = 0;

    double ye_value = ye - (int)roundDouble(IszX / 2.0);
    if (ye_value < 0)
      ye_value = 0;

    double distance =
        sqrt(pow((double)(xe_value), 2) + pow((double)(ye_value), 2));
    printf("%lf\n", distance);
