...
double normalize(double in, double min, double max, double min_new,
                 double max_new) {
...
#pragma omp single
    {
#pragma omp approx taskloop drop((int)(10 * DROP)) private(i, j) collapse(2)
      {
        for (j = 0; j < NUM_RUNS; j++) {
          for (i = start; i < end; i++) {
            ...
            price_orig = BlkSchlsEqEuroNoDiv(sptprice[i], strike[i], rate[i],
